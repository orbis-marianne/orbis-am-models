"""
Statement Classification Datasets. It has the definitions of the
`StatementClassificationDataset` and the `StatementClassificationDataModule`.
The Dataset reads column based data (csv, tsv) for Statement Classification.

    Argumentation Mining Transformers Statement Classification Data Module
...
class StatementClassificationDataset(BaseDataset):
    """
    Dataset for classification of argumentative statements (e.g. position,
    ...
    def _load_dataset(
        ...
        **kwargs,
        ...
        self.dataset = [{"text": d[1]} for d in dataset]
...
class StatementClassificationDataModule(BaseDataModule):
    ...
    LABELS = ["Position", "Evidence"] #"Support", "Attack"
    ...
    def decode_predictions(
        ...
        batch of instances (List[List[int]]) into its corresponding sentence. It
        appends the predicted labels (using id2label) and, if present, the true
        labels.
        ...
        Tuple[str], List[Tuple[str]]
            Depending on the type of input, it can return a single tuple or a
            list of tuples (if a batch of instances). The tuple has the form:
            (predicted_label, sentence1) or (true_label, predicted_label,
            sentence1) if the true label is given.
        """
        if isinstance(input_ids[0], int):
            ...
            if labels is not None:
                ...
                return true_label, predicted_label, sentence
            else:
                return predicted_label, sentence
        else:
            # Batch of inputs
            if labels is None:
                return [
    ...
    def _load_dataset_split(self, path_to_dataset: str) -> StatementClassificationDataset:
        """
        Check BaseDataModule for documentation.
        """
        return StatementClassificationDataset(
